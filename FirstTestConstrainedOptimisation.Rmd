...
This might be the trickiest bit, as this requires introducing a parameter $\beta$, but also to calibrate on this. Originally I wanted to rewrite the hydromad optimisation function, but I have started much easier by defining my own function that can be optimised using the standard optim function. In principle this assumes that the constraints:  
$error < \beta$  
and  
$error > -\beta$  
Can be rewritten as:
$\sqrt{(\sqrt(error^2) - \beta)^2}$
...
objfun <- function(par, in_data, mod, ...) {
  ...
  adj_error_Q <- sqrt((sqrt(error_Q^2) - par[6])^2)
